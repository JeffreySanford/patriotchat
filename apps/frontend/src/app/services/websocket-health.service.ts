...
export class WebSocketHealthService {
  ...
  private healthStatus$: Subject<HealthCheckEvent> =
    new Subject<HealthCheckEvent>();
  private healthError$: Subject<HealthCheckError> =
    new Subject<HealthCheckError>();
  private connected$: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(
    false,
  );
  private socketUrl: string = 'http://localhost:3000';
  private socketNamespace: string = 'health';

  /**
   * Connect to health WebSocket and return observable of health events
   */
  connect(): Observable<HealthCheckEvent> {
    return new Observable<HealthCheckEvent>(
      (observer: Observer<HealthCheckEvent>) => {
        ...
        this.socket.on('disconnect', () => {
          console.log(
            '[WebSocketHealthService] Disconnected from health WebSocket',
          );
        ...
        this.socket.on(
          'health-status',
          (event: HealthCheckEvent | HealthCheckError) => {
            try {
              if (this.isHealthCheckEvent(event)) {
                this.validateHealthCheckEvent(event);
                console.log(
                  '[WebSocketHealthService] Received health status:',
                  event,
                );
                observer.next(event);
              ...
              console.error(
                '[WebSocketHealthService] DTO Validation failed:',
                validationError,
              );
        ...
        this.socket.on(
          'connect_error',
          (error: Error | Record<string, string | number>): void => {
  ...
  private subscribeToEvents(observer: Observer<HealthCheckEvent>): void {
  ...
  private isHealthCheckEvent(
    ...
    return (
      ...
      Array.isArray((event as Record<string, unknown>)['services']) &&
      // eslint-disable-next-line no-restricted-syntax
   ...
   */
  private isHealthCheckError(
    event:
      | HealthCheckEvent
      | HealthCheckError
      | Record<string, string | number>,
  ): event is HealthCheckError {
    return (
      event &&
      typeof event === 'object' &&
      'statusCode' in event &&
  ...
  private validateHealthCheckEvent(event: HealthCheckEvent): void {
    ...
    event.services.forEach((service: ServiceStatus, index: number): void => {
