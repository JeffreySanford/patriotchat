# Debug Documentation Moved

All debug documentation has been organized into the `documentation/debug/` folder.

## 👉 Start Here

→ **[documentation/debug/DEBUG_LOGGING_INDEX.md](documentation/debug/DEBUG_LOGGING_INDEX.md)** - Complete Reference & Navigation

## Quick Links

| Document                                                              | Purpose                  | Read Time |
| --------------------------------------------------------------------- | ------------------------ | --------- |
| **[Quick Start](documentation/debug/DEBUGGING_QUICK_START.md)**       | Get started in 2 minutes | 2 min     |
| **[Complete Guide](documentation/debug/DEBUG_LOGGING_GUIDE.md)**      | Comprehensive reference  | 10 min    |
| **[Error Reference](documentation/debug/DEBUG_ERRORS_REFERENCE.md)**  | Troubleshooting guide    | 5 min     |
| **[Changes Summary](documentation/debug/CHANGES_SUMMARY.md)**         | What was changed         | 3 min     |
| **[CORS & WebSocket Fix](documentation/debug/CORS_WEBSOCKET_FIX.md)** | Configuration fix        | 3 min     |

---

## About These Docs

These documents provide complete visibility into the PatriotChat query pipeline:

- ✅ Frontend → Backend query flow
- ✅ WebSocket stage updates
- ✅ Performance metrics
- ✅ Common errors and solutions
- ✅ CORS & WebSocket configuration

**All files have been moved to:** `documentation/debug/`

**New to debugging?** Start with [DEBUGGING_QUICK_START.md](documentation/debug/DEBUGGING_QUICK_START.md)

**Have an error?** Check [DEBUG_ERRORS_REFERENCE.md](documentation/debug/DEBUG_ERRORS_REFERENCE.md)

**Want all details?** Read [DEBUG_LOGGING_GUIDE.md](documentation/debug/DEBUG_LOGGING_GUIDE.md)
