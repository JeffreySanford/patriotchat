...
### 8. **LLM Training Stack Spec**

This spec captures the runtime dependencies and recent progress for the pro-liberty LLM sprint; every entry references the Values Commitment (`README.md#values-commitment`) and the build roadmap (`documentation/planning/pro-liberty/PRO_LIBERTY_BUILD_GUIDE.md`) so the lone developer + AI workflow can trace install decisions back to those constitutional guardrails.

- **Node/pnpm availability** – The WSL build now exposes `node v18.19.1`, `npm 9.2.0`, and the `pnpm` shim under `/mnt/c/Users/Sanford/AppData/Local/pnpm/pnpm`, so `pnpm run generate:liberty-prompts` can transpile the TypeScript generator (`scripts/pro_liberty_prompt_generator.ts`) and call `accelerate` without the earlier “node: not found” failure.
- **Python stack (axolotl-env312 at `/home/jeffrey/axolotl-env312`)** – After installing Ubuntu 24.04’s `python3.12`, we created a dedicated venv at `/home/jeffrey/axolotl-env312`, upgraded `pip`, and installed `numpy==2.0.1 scikit-learn==1.4.2 accelerate axolotl bitsandbytes peft trl`. The missing `coverage.types` attribute was resolved by pinning `coverage==7.8.0`. This stack now satisfies `triton>=3.0.0` and `bitsandbytes` via the manylinux builds, so the previous blockers reported in this section are cleared.
- **Training config** – `liberty-mistral-lora.yaml` lives at the repo root and encodes the Values Commitment style (messages/chat dataset, enumerated powers prompts) while pointing at `my_liberty_dataset/train.jsonl`. The YAML declares `type: messages.chat`, `field_messages: messages`, `message_property_mappings` for `role`/`content`, LoRA hyperparameters, `max_steps: 1` (for quick sanity checks), and `report_to: none` with `use_wandb: false`/`wandb_mode: disabled` so `transformers` won’t prompt for a WandB key. The script log clearly shows the dataset being tokenized, sample packing configured, and the trainer beginning even though the full run is long.
- **Command status** – Running `/home/jeffrey/axolotl-env312/bin/accelerate launch -m axolotl.cli.train liberty-mistral-lora.yaml --report_to none` now begins the training pipeline (dataset packing, quantization prep, adapter saving) rather than immediately erroring. A long dataset prep still runs for several minutes (the latest timed-out run was killed after ~5 minutes once the quick `max_steps=1` job started), but no additional dependency errors remain; future loops can let the job complete once scheduling allows. Log the `accelerate` command and the new YAML file in `documentation/planning/pro-liberty/PRO_LIBERTY_TRACKING.md` so every sprint item references the Values Commitment and the Build Guide.
- **Artifact bundle** – After the quick run finished packing, the adapter/tokenizer artifacts were zipped as `tools/checkpoints/liberty-mistral-v1.0-2026-02-04/liberty-mistral-v1.0-2026-02-04.zip` (with `adapter_config.json` beside it); the promoted second pass lives under `tools/checkpoints/liberty-mistral-v1.0-2026-02-05/` (with metadata.json, README, and the zipped bundle) and should be referenced along with `documentation/planning/pro-liberty/PRO_LIBERTY_ALIGNMENT_TESTS.md` when evaluating the bundle, then note the evaluation/sanity metrics back in `documentation/planning/pro-liberty/PRO_LIBERTY_TRACKING.md`.
